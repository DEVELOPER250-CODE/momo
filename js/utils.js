var MOMO_Utils = {
    ...
    validPhone: function(p) {
        return /^7[89]\d{7}$/.test((p || '').replace(/\s/g, ''));
    },
    fmtAmount: function(a) {
        return parseInt(a).toLocaleString() + ' RWF';
    },
    ...
    getFreeWeeksRemaining: function() {
        try {
            ...
            return Math.max(0, Math.ceil((d.freeUntil - Date.now()) / (1000 * 60 * 60 * 24 * 7)));
        } catch(e) { return 0; }
    },
    saveContrib: function(amount, ref, weeks) {
        var proof = {
            ...
            weeks: weeks,
            at: Date.now(),
            freeUntil: Date.now() + (weeks * 7 * 24 * 60 * 60 * 1000)
        };
